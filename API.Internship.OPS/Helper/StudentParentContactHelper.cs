...
{
    public interface IStudentParentContactHelper
    {
    ...
    public class StudentParentContactHelper: IStudentParentContactHelper
    {
        private readonly ILogger<StudentParentContactHelper> _logger;
        ...
        public async Task<R_Data> MergeData(R_Data res)
        {
            try
            {
                ...
                {
                    StudentParentContact StudentParentContactObj = res.data;
                    Type myType = StudentParentContactObj.GetType();
                    IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
                    foreach (PropertyInfo prop in props)
                    {
                        dict.Add(prop.Name, prop.GetValue(StudentParentContactObj));
                    }
                    dict.Add("StudentObj", new Dictionary<string, dynamic>());
                    R_Data resStudent = _personService.GetAsync((int)StudentParentContactObj.StudentId).Result;
        ...
        public async Task<R_Data> MergeDataList(R_Data res)
        {
            ...
            try
            {
                if (res.result == 1 && res.data != null)
                {
                    List<StudentParentContact> StudentParentContactObjs = res.data;
                    StudentParentContactObjs.ForEach(StudentParentContactObj =>
                    {
                        Type myType = StudentParentContactObj.GetType();
                        IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
                        Dictionary<string, dynamic> dict = new Dictionary<string, dynamic>();
        ...
        public async Task<R_Data> MergeDynamicList(R_Data res)
        {
            ...
            try
            {
                if (res.result == 1 && res.data != null)
                {
                    ...
                    var StudentParentContactObjs = res.data;
                    foreach (var StudentParentContactObj in StudentParentContactObjs)
                    {
                        Dictionary<string, dynamic> dict = new Dictionary<string, dynamic>();
                        Type myType = StudentParentContactObj.GetType();
                        IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
