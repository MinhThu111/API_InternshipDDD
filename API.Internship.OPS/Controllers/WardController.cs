...
namespace API.Internship.OPS.Controllers
{
    ...
    public class WardController : ControllerBase
    {
        ...
        public async Task<ActionResult<R_Data>> Update(Ward item)
        {
            ...
            try
            {
                res = await _wardService.PutAsync(item.Id, item.Name1, item.NameSlug, item.WardCode, item.UpdatedBy, item.Timer);
                res = await _wardHelper.MergeData(res);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<R_Data>> Create(Ward item)
        {
            ...
            try
            {
                res = await _wardService.PutAsync(item.Name1, item.NameSlug, item.WardCode, item.DistrictId);
                res = await _wardHelper.MergeData(res);
            }
            catch (Exception ex)
            {
