...
namespace API.Internship.Infrastructure.Repositories
{
    ...
    {
        public NewsCategoryRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.NewsCategories.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
        ...
        public async Task<NewsCategory> GetId(int id)
        {
            var query = GetAll().Where(w => w.Status != -1).AsNoTracking().FirstOrDefault(f => f.Id == id);
            return await Task.Run(() => query);
        }
        public NewsCategory Delete(NewsCategory obj)
        {
            try
            {
                obj.Status = -1; //xóa
                ...
                _context.NewsCategories.Update(obj);
            }
            catch (Exception) { }
            return obj;
        }
