...
{
    public class FolkRepository:GenericRepository<Folk>, IFolkRepository
    {
        public FolkRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.Folks.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
