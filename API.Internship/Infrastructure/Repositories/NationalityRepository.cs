...
namespace API.Internship.Infrastructure.Repositories
{
    ...
    {
        public NationalityRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.Nationalities.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
        ...
        public Nationality Delete(Nationality obj)
        {
            try
            {
                obj.Status = -1; //xóa
                ...
                _context.Nationalities.Update(obj);
            }
            catch (Exception) { }
            return obj;
        }
