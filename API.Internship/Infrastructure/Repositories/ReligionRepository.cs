...
namespace API.Internship.Infrastructure.Repositories
{
    ...
    {
        public ReligionRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.Religions.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
