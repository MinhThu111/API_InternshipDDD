...
namespace API.Internship.Infrastructure.Repositories
{
    ...
    {
        public WardRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.Wards.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
