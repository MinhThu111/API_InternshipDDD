...
{
    public class ParentContactRepository : GenericRepository<ParentContact>, IParentContactRepository
    {
        public ParentContactRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.ParentContacts.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
        ...
        public async Task<ParentContact> GetId(int id)
        {
            var query = GetAll().Where(w => w.Status != -1).AsNoTracking().FirstOrDefault(f => f.Id == id);
            return await Task.Run(() => query);
        }
        public ParentContact Delete(ParentContact obj)
        {
            try
            {
                obj.Status = -1; //xóa
                ...
                _context.ParentContacts.Update(obj);
            }
            catch (Exception) { }
            return obj;
        }
