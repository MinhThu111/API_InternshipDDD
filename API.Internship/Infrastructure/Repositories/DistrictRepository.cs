...
namespace API.Internship.Infrastructure.Repositories
{
    ...
    {
        public DistrictRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.Districts.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
