...
{
    public class SchoolInfoRepository: GenericRepository<SchoolInfo>, ISchoolInfoRepository
    {
        public SchoolInfoRepository(InternshipContext context) : base(context) { }
        public async Task<internalData> Max()
        {
            ...
            try
            {
                var max = await _context.SchoolInfos.OrderByDescending(m => m.Id).FirstOrDefaultAsync();
        ...
        public async Task<SchoolInfo> GetId(int id)
        {
            var query = GetAll().Where(w => w.Status != -1).AsNoTracking().FirstOrDefault(f => f.Id == id);
            return await Task.Run(() => query);
        }
        public SchoolInfo Delete(SchoolInfo obj)
        {
            try
            {
                obj.Status = -1; //xóa
                ...
                _context.SchoolInfos.Update(obj);
            }
            catch (Exception) { }
            return obj;
        }
